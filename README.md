...
- **Log Sanitization**: Uses GitHub Actions secret masking (`@actions/core.setSecret`) to prevent leakage of source names (including URL-encoded names), private URLs, tokens, and credentials into action execution logs. Source names appear as `***` in workflow logs; this masking is applied by the GitHub Actions runner, not local runs.
