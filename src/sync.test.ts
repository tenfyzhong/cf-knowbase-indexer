...
import { generateVectorId } from "./chunker.js";
...
describe("sync diff and engine", () => {
  ...
  it("should orchestrate syncSource workflow correctly via KnowbaseApiClient", async () => {
    ...
    expect(mockClient.deleteVectors).toHaveBeenCalledWith([
      generateVectorId("test-src", "old.md", 0),
      generateVectorId("test-src", "mod.md", 0),
      generateVectorId("test-src", "mod.md", 1)
    ]);
  ...
  it("should NOT vectorize documents containing #confidential tag", async () => {
    ...
    expect(mockClient.upsertChunks).toHaveBeenCalledWith([
      expect.objectContaining({
        id: generateVectorId("obsidian", "public-note.md", 0),
        path: "public-note.md"
      })
    ]);
  });
  ...
  it("should delete existing vectors when a document is modified to include #confidential tag", async () => {
    ...
    expect(mockClient.deleteVectors).toHaveBeenCalledWith([
      generateVectorId("obsidian", "note.md", 0),
      generateVectorId("obsidian", "note.md", 1)
    ]);
